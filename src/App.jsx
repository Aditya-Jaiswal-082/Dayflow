...
function offsetKey(k,n) {
  ...
  dt.setDate(dt.getDate()+n); return makeKey(dt);
}
function fmtKey(k) {
  const [y,m,d]=k.split("-"), dt=new Date(+y,+m-1,+d);
...
function past7(fromKey) {
  ...
  return Array.from({length:7},(_,i)=>{
    const dt=new Date(+y,+m-1,+d); dt.setDate(dt.getDate()-6+i); return makeKey(dt);
  });
}
...
function pctColor(p) { return p>=80?"#10b981":p>=50?"#6366f1":"#f59e0b"; }
...
/* ── pure sub-components (outside App — never recreated) ───────────────────── */
function Sheet({ open, onClose, title, children }) {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.55)",display:"flex",alignItems:"flex-end"}}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",background:"var(--card)",borderRadius:"22px 22px 0 0",maxHeight:"92vh",overflowY:"auto",animation:"slideUp .28s cubic-bezier(.4,0,.2,1)"}}>
        <div style={{display:"flex",justifyContent:"center",padding:"12px 0 4px"}}>
          <div style={{width:36,height:4,borderRadius:2,background:"var(--border)"}}/>
        </div>
        {title && <div style={{padding:"6px 20px 12px",fontWeight:700,fontSize:17,color:"var(--text)"}}>{title}</div>}
...
function Checkbox({ checked, onChange, color="#6366f1" }) {
  return (
    <div onClick={onChange} style={{width:23,height:23,borderRadius:7,flexShrink:0,cursor:"pointer",border:`2px solid ${checked?color:"var(--border)"}`,background:checked?color:"transparent",display:"flex",alignItems:"center",justifyContent:"center",transition:"all .18s"}}>
  ...
  return (
    <div style={{position:"relative",width:size,height:size,flexShrink:0}}>
      <svg width={size} height={size} style={{transform:"rotate(-90deg)",position:"absolute",inset:0}}>
        ...
        <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={color} strokeWidth={stroke} strokeDasharray={`${(pct/100)*c} ${c}`} strokeLinecap="round" style={{transition:"stroke-dasharray .5s ease"}}/>
      </svg>
...
function Lbl({ children }) {
  return <div style={{fontSize:11,fontWeight:600,color:"var(--muted)",textTransform:"uppercase",letterSpacing:.6,marginBottom:6}}>{children}</div>;
}

function Sec({ children, action }) {
  return (
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
      <div style={{fontSize:12,fontWeight:600,color:"var(--muted)",textTransform:"uppercase",letterSpacing:.7}}>{children}</div>
      {action}
    </div>
  );
}

function Toggle({ on, onChange, border }) {
  return (
    <div onClick={onChange} style={{width:44,height:26,borderRadius:13,position:"relative",cursor:"pointer",transition:"background .2s",flexShrink:0,background:on?"#6366f1":border}}>
      <div style={{position:"absolute",top:3,left:on?21:3,width:20,height:20,borderRadius:"50%",background:"#fff",transition:"left .2s",boxShadow:"0 1px 3px rgba(0,0,0,.2)"}}/>
    </div>
  );
}
...
const INIT_NOTES = [
  {id:201,title:"Ideas for the week",body:"- Learn something new\n- Call family\n- Plan weekend",color:"#6366f1",pinned:true,created:todayKey(),tags:["personal"]},
];
...
export default function App() {
  ...
  const [dayData,  setDayData]  = useLS("df_daydata",  {});
  const [expenses, setExpenses] = useLS("df_expenses", []);
  ...
  const patchDay = useCallback((k,patch) =>
    setDayData(p=>({...p,[k]:{...(p[k]||EMPTY_DAY()),...patch}})), [setDayData]);
  ...
  const doneView  = reg.filter(t=>curDay.checks?.[t.id]).length+(curDay.extra||[]).filter(x=>x.done).length+(curDay.special||[]).filter(x=>x.done).length;
  ...
  const filteredNotes = notes
    .filter(n=>!noteSearch||n.title.toLowerCase().includes(noteSearch.toLowerCase())||n.body.toLowerCase().includes(noteSearch.toLowerCase())||(n.tags||[]).some(t=>t.toLowerCase().includes(noteSearch.toLowerCase())))
    .sort((a,b)=>(b.pinned?1:0)-(a.pinned?1:0));

  /* ── actions ───────────────────────────────────────────────────────────── */

  // Tasks
  function openAddTask()    { setTaskForm(blankTask()); setEditTaskId(null); setSheet("task"); }
  function openEditTask(t)  { setTaskForm({text:t.text}); setEditTaskId(t.id); setSheet("task"); }
  function saveTask() {
    if (!taskForm.text.trim()) return;
    if (editTaskId) {
      setTasks(p=>p.map(t=>t.id===editTaskId?{...t,text:taskForm.text.trim()}:t));
      setEditTaskId(null);
    } else {
  ...
  // Expenses
  function openAddExp()   { setExpForm(blankExp()); setEditExpId(null); setSheet("expense"); }
  function openEditExp(e) { setExpForm({amount:String(e.amount),desc:e.desc,cat:e.cat,date:e.date,note:e.note,split:e.split,splitWith:e.splitWith}); setEditExpId(e.id); setSheet("expense"); }
  function saveExp() {
    if (!expForm.amount||!expForm.desc.trim()) return;
    if (editExpId) {
      setExpenses(p=>p.map(e=>e.id===editExpId?{...e,...expForm,amount:+expForm.amount}:e));
      setEditExpId(null);
    } else {
  ...
  function deleteExp(id) { setExpenses(p=>p.filter(e=>e.id!==id)); }

  // Notes
  function openAddNote()   { setNoteForm(blankNote()); setEditNoteId(null); setSheet("note"); }
  function openEditNote(n) { setNoteForm({title:n.title,body:n.body,color:n.color,tags:(n.tags||[]).join(", ")}); setEditNoteId(n.id); setSheet("note"); }
  function saveNote() {
    if (!noteForm.body.trim()) return;
    const tags=noteForm.tags.split(",").map(t=>t.trim()).filter(Boolean);
    if (editNoteId) {
      setNotes(p=>p.map(n=>n.id===editNoteId?{...n,...noteForm,tags}:n));
      setEditNoteId(null);
    } else {
      setNotes(p=>[{id:Date.now(),title:noteForm.title,body:noteForm.body,color:noteForm.color,pinned:false,created:todayKey(),tags},...p]);
    }
  ...
  function taskReminder(text,hrs) { setTimeout(()=>alert(`⏰ Reminder: "${text}"`),hrs*3600*1000); }
  ...
  const css=`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
    ...
    .btn-red{background:#ef4444;color:#fff;border-radius:14px;padding:14px;font-size:15px;font-weight:600;width:100%;display:block;transition:opacity .15s}
  ...
  /* ── NoteCard (inside App to access openEditNote) ──────────────────────── */
  function NoteCard({ n }) {
    return (
      <div onClick={()=>openEditNote(n)} style={{background:n.color+"15",border:`1.5px solid ${n.color}30`,borderRadius:16,padding:"14px",cursor:"pointer",position:"relative",minHeight:100}}>
        <div style={{position:"absolute",top:10,right:10}}>
          <button onClick={e=>{e.stopPropagation();setNotes(p=>p.map(x=>x.id===n.id?{...x,pinned:!x.pinned}:x));}} style={{fontSize:14,opacity:n.pinned?1:.3,transition:"opacity .2s"}}>📌</button>
        </div>
        <div style={{width:10,height:10,borderRadius:"50%",background:n.color,marginBottom:8}}/>
        {n.title&&<div style={{fontSize:14,fontWeight:700,color:text,marginBottom:5,paddingRight:24,lineHeight:1.3}}>{n.title}</div>}
        <div style={{fontSize:13,color:muted,lineHeight:1.55,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:4,WebkitBoxOrient:"vertical"}}>{n.body}</div>
        {(n.tags||[]).length>0&&(
          <div style={{display:"flex",flexWrap:"wrap",gap:4,marginTop:10}}>
            {n.tags.map(t=><span key={t} style={{fontSize:10,fontWeight:600,padding:"2px 6px",borderRadius:99,background:n.color+"25",color:n.color}}>#{t}</span>)}
          </div>
        )}
  ...
  function DayNav() {
    ...
    return (
      <div style={{display:"flex",alignItems:"center",background:card,borderRadius:14,border:`1px solid ${border}`,overflow:"hidden",marginBottom:14}}>
        <button onClick={()=>setViewKey(k=>offsetKey(k,-1))} style={{padding:"10px 16px",color:muted,fontSize:20,lineHeight:1}}>‹</button>
        <div style={{flex:1,textAlign:"center",padding:"8px 0"}}>
          <div style={{fontSize:14,fontWeight:700,color:text}}>{isToday?"Today":fmtKey(viewKey)}</div>
        </div>
        <button onClick={()=>setViewKey(k=>offsetKey(k,1))} disabled={future} style={{padding:"10px 16px",color:future?border:muted,fontSize:20,lineHeight:1}}>›</button>
        {!isToday&&(
          <button onClick={()=>setViewKey(tk)} style={{padding:"10px 14px",fontSize:11,fontWeight:600,color:"#6366f1",borderLeft:`1px solid ${border}`}}>Today</button>
        )}
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div style={{paddingBottom:90}}>
        ...
        <div style={{padding:"52px 20px 0",display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
          <div>
            ...
            <div style={{fontSize:24,fontWeight:700,marginTop:2,letterSpacing:-.5}}>{getGreeting()} 👋</div>
          </div>
          <div style={{display:"flex",gap:8,marginTop:6}}>
            ...
            <button onClick={()=>setDark(d=>!d)} style={{background:card,border:`1px solid ${border}`,borderRadius:12,width:36,height:36,fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>{dark?"☀️":"🌙"}</button>
          </div>
        </div>

        {/* Quote */}
        <div style={{padding:"12px 20px 0"}}>
          <div style={{background:"linear-gradient(135deg,#6366f1,#8b5cf6)",borderRadius:16,padding:"11px 16px"}}>
            <div style={{fontSize:13,color:"rgba(255,255,255,0.92)",fontStyle:"italic",lineHeight:1.55}}>"{quote[0]}"</div>
        ...
        <div style={{padding:"12px 20px 0",display:"flex",gap:6}}>
          {[["today","Today"],["stats","Stats"],["expenses","Wallet"],["notes","Notes"]].map(([id,label])=>(
            <button key={id} onClick={()=>setTab(id)} style={{flex:1,padding:"9px 4px",borderRadius:12,fontSize:13,fontWeight:600,background:tab===id?"#6366f1":card,color:tab===id?"#fff":muted,border:`1px solid ${tab===id?"transparent":border}`,transition:"all .2s"}}>{label}</button>
          ))}
        </div>
        ...
        <div style={{padding:"12px 20px 0"}} className="fade" key={tab}>
            ...
            <>
              ...
              <div className="card" style={{display:"flex",gap:16,alignItems:"center",marginBottom:12}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:13,color:muted,marginTop:2}}>{viewPct===100?"All done! 🎉":viewPct>=50?"Halfway there 💪":"Let's get started 🚀"}</div>
                  <div style={{marginTop:10,height:5,background:border,borderRadius:99,overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${viewPct}%`,background:pctColor(viewPct),borderRadius:99,transition:"width .5s ease"}}/>
                  </div>
                </div>
              </div>
              ...
              <div className="card" style={{marginBottom:12}}>
                <Sec action={<button onClick={openAddTask} style={{background:"#6366f1",color:"#fff",borderRadius:10,padding:"5px 13px",fontSize:12,fontWeight:600}}>+ Add</button>}>Daily tasks</Sec>
                {reg.length===0&&<div style={{textAlign:"center",padding:"16px 0",color:muted,fontSize:14}}>No tasks yet — add your first!</div>}
                {reg.map((t,i)=>(
                  <div key={t.id} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 0",borderBottom:i<reg.length-1?`1px solid ${border}`:"none"}}>
                    <Checkbox checked={!!curDay.checks?.[t.id]} onChange={()=>patchDay(viewKey,{checks:{...curDay.checks,[t.id]:!curDay.checks?.[t.id]}})}/>
                    <span style={{flex:1,fontSize:15,textDecoration:curDay.checks?.[t.id]?"line-through":"none",color:curDay.checks?.[t.id]?muted:text,transition:"all .2s"}}>{t.text}</span>
                    {/* Edit & delete */}
                    <button onClick={()=>openEditTask(t)} style={{color:muted,fontSize:13,padding:"2px 6px",border:`1px solid ${border}`,borderRadius:7,lineHeight:1.4}}>✏️</button>
                    <button onClick={()=>deleteTask(t.id)} style={{color:muted,fontSize:17,lineHeight:1}}>×</button>
                  </div>
                ))}
              </div>
                ...
                <div className="card" style={{marginBottom:12}}>
                  ...
                  {(curDay.extra||[]).map((t,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 0",borderBottom:i<curDay.extra.length-1?`1px solid ${border}`:"none"}}>
                      ...
                      <button onClick={()=>{const a=[...curDay.extra];a.splice(i,1);patchDay(viewKey,{extra:a});}} style={{color:muted,fontSize:17,lineHeight:1}}>×</button>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div className="card" style={{marginBottom:12}}>
                  ...
                  {(curDay.special||[]).map((t,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 0",borderBottom:i<curDay.special.length-1?`1px solid ${border}`:"none"}}>
                      ...
                      <div style={{flex:1}}>
                        <span style={{fontSize:15,textDecoration:t.done?"line-through":"none",color:t.done?muted:text}}>{t.text}</span>
                        {t.dueDate&&t.dueDate!==viewKey&&<div style={{fontSize:11,color:"#ef4444",marginTop:2}}>Due {fmtKey(t.dueDate)}</div>}
                      </div>
                      <button onClick={()=>{const a=[...curDay.special];a.splice(i,1);patchDay(viewKey,{special:a});}} style={{color:muted,fontSize:17,lineHeight:1}}>×</button>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                ...
                <button onClick={()=>setSheet("addPriority")} style={{background:card,border:"1.5px dashed #ef444440",borderRadius:14,padding:"13px",fontSize:13,fontWeight:600,color:"#ef4444",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>⭐ Priority task</button>
              </div>
              ...
              {curDay.note&&(
                <div className="card" style={{marginTop:10,borderLeft:"3px solid #6366f1"}}>
                  ...
                  <div style={{fontSize:14,color:text,lineHeight:1.6,whiteSpace:"pre-wrap"}}>{curDay.note}</div>
                </div>
              )}
            </>
          )}
                  ...
                  return (
                    <div key={k} onClick={()=>{setViewKey(k);setTab("today");}} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:`1px solid ${border}`,cursor:"pointer"}}>
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{fontSize:12,color:muted,marginTop:1}}>{nd.note?"Has note · ":""}{reg.filter(t=>nd.checks?.[t.id]).length}/{reg.length} regular done</div>
                      </div>
          ...
          {tab==="expenses"&&(
            <>
              ...
              <button className="dash" onClick={openAddExp} style={{marginBottom:12,color:"#6366f1",borderColor:"#6366f130"}}>+ Add expense</button>
              ...
              {splitExp.length>0&&(
                <div className="card" style={{marginBottom:12,border:`1px solid #6366f130`}}>
                  ...
                  {splitExp.map((e,i)=>(
                    <div key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<splitExp.length-1?`1px solid ${border}`:"none"}}>
                      <div style={{width:36,height:36,borderRadius:10,flexShrink:0,background:catColor(e.cat)+"18",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>{catIcon(e.cat)}</div>
                      <div style={{flex:1}}>
                  ...
                  {expByCat.map(c=>(
                    <div key={c.id} style={{marginBottom:11}}>
                      ...
                      <div style={{height:5,background:border,borderRadius:99,overflow:"hidden"}}>
                        <div style={{height:"100%",width:`${Math.round((c.total/totalMonth)*100)}%`,background:c.color,borderRadius:99,transition:"width .4s ease"}}/>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="card">
                ...
                {expenses.length===0&&<div style={{textAlign:"center",padding:"20px 0",color:muted,fontSize:14}}><div style={{fontSize:30,marginBottom:8}}>💸</div>No expenses yet</div>}
                {expenses.slice(0,40).map((e,i)=>(
                  <div key={e.id} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 0",borderBottom:i<Math.min(expenses.length,40)-1?`1px solid ${border}`:"none"}}>
                    <div style={{width:44,height:44,borderRadius:12,flexShrink:0,background:catColor(e.cat)+"18",display:"flex",alignItems:"center",justifyContent:"center",fontSize:21}}>{catIcon(e.cat)}</div>
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{fontSize:12,color:muted,marginTop:1,display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
                        ...
                        {e.split&&<span style={{background:"#6366f118",color:"#6366f1",borderRadius:6,padding:"0 5px",fontSize:10,fontWeight:600}}>Split w/ {e.splitWith}</span>}
                      </div>
                    ...
                    <div style={{textAlign:"right",flexShrink:0}}>
                      ...
                      {e.split&&<div style={{fontSize:10,color:"#6366f1"}}>₹{(e.amount/2).toLocaleString()} each</div>}
                      {/* Edit & delete */}
                      <div style={{display:"flex",gap:6,justifyContent:"flex-end",marginTop:4}}>
                        <button onClick={()=>openEditExp(e)} style={{fontSize:11,color:"#6366f1",fontWeight:600,border:`1px solid #6366f140`,borderRadius:6,padding:"2px 7px"}}>Edit</button>
            ...
            <>
              <div style={{display:"flex",gap:8,marginBottom:14}}>
                <div style={{flex:1,position:"relative"}}>
                  ...
                  <input value={noteSearch} onChange={e=>setNoteSearch(e.target.value)} placeholder="Search notes, tags..." style={{paddingLeft:36}}/>
                </div>
                <button onClick={openAddNote} style={{background:"#6366f1",color:"#fff",borderRadius:12,padding:"0 18px",fontSize:20,fontWeight:600,flexShrink:0}}>+</button>
              </div>
              ...
              {filteredNotes.length===0&&(
                <div style={{textAlign:"center",padding:"48px 0",color:muted}}>
                  ...
                  <div style={{fontSize:13,marginTop:4}}>{noteSearch?"Try a different search":"Tap + to write your first note"}</div>
                </div>
              )}
      ...
      {/* Add / Edit daily task */}
      <Sheet open={sheet==="task"} onClose={()=>setSheet(null)} title={editTaskId?"Edit task":"Add daily task"}>
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <input value={taskForm.text} onChange={e=>setTaskForm(p=>({...p,text:e.target.value}))} placeholder="What do you need to do every day?" autoFocus/>
          <button className="btn-p" onClick={saveTask}>{editTaskId?"Save changes":"Add task"}</button>
          {editTaskId&&(
            <button onClick={()=>{deleteTask(editTaskId);setSheet(null);}} style={{color:"#ef4444",fontSize:14,fontWeight:600,padding:8}}>Delete task</button>
          )}
        </div>
      </Sheet>
      ...
      <Sheet open={sheet==="addExtra"} onClose={()=>setSheet(null)} title="Extra task for today">
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <button className="btn-p" onClick={()=>{if(!taskForm.text.trim())return;patchDay(viewKey,{extra:[...(curDay.extra||[]),{text:taskForm.text.trim(),done:false}]});setTaskForm(blankTask());setSheet(null);}}>Add</button>
        </div>
      </Sheet>
      ...
      <Sheet open={sheet==="addPriority"} onClose={()=>setSheet(null)} title="⭐ Priority task">
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:inp,borderRadius:12,padding:"12px 14px"}}>
            <div>
              <div style={{fontSize:15,fontWeight:500}}>Set reminder</div>
              <div style={{fontSize:12,color:muted}}>Get notified before due time</div>
            </div>
            <Toggle on={priorityForm.reminder} onChange={()=>setPriorityForm(p=>({...p,reminder:!p.reminder}))} border={border}/>
          </div>
          {priorityForm.reminder&&(
            <div><Lbl>Remind me in</Lbl>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                {[1,3,5].map(h=>(
                  <button key={h} onClick={()=>setPriorityForm(p=>({...p,reminderHrs:h}))} style={{padding:"12px 0",borderRadius:12,background:priorityForm.reminderHrs===h?"#ef4444":inp,border:`1.5px solid ${priorityForm.reminderHrs===h?"#ef4444":border}`,color:priorityForm.reminderHrs===h?"#fff":text,fontSize:16,fontWeight:700,transition:"all .15s"}}>
          ...
          <button className="btn-red" onClick={()=>{
            ...
            patchDay(priorityForm.date,{special:[...(getDay(priorityForm.date).special||[]),task]});
            if(priorityForm.reminder) taskReminder(priorityForm.text.trim(),priorityForm.reminderHrs);
            setPriorityForm(blankPri());
            setSheet(null);
      ...
      <Sheet open={sheet==="dayNote"} onClose={()=>setSheet(null)} title="Day note">
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <textarea value={curDay.note||""} onChange={e=>patchDay(viewKey,{note:e.target.value})} placeholder="How was your day? Thoughts, reflections..." rows={7} autoFocus/>
      ...
      <Sheet open={sheet==="reminder"} onClose={()=>setSheet(null)} title="Set reminder">
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
            {[1,3,5].map(h=>(
              <button key={h} onClick={()=>setRemHrs(h)} style={{padding:"16px 0",borderRadius:14,background:remHrs===h?"#6366f1":inp,border:`1.5px solid ${remHrs===h?"#6366f1":border}`,color:remHrs===h?"#fff":text,fontSize:18,fontWeight:700,transition:"all .15s"}}>
                {h}<span style={{fontSize:13,fontWeight:500}}>h</span>
              </button>
            ))}
          </div>
          <button className="btn-p" onClick={startReminder}>Set reminder</button>
          {reminder.active&&<button onClick={cancelReminder} style={{color:"#ef4444",fontSize:14,fontWeight:600,padding:8}}>Cancel reminder</button>}
        </div>
      </Sheet>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
              {EXP_CATS.map(c=>(
                <button key={c.id} onClick={()=>setExpForm(p=>({...p,cat:c.id}))} style={{padding:"10px 4px",borderRadius:12,background:expForm.cat===c.id?c.color+"22":inp,border:`1.5px solid ${expForm.cat===c.id?c.color:border}`,display:"flex",flexDirection:"column",alignItems:"center",gap:3,transition:"all .15s"}}>
          ...
          <div style={{background:inp,borderRadius:12,padding:"12px 14px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              ...
              <Toggle on={expForm.split} onChange={()=>setExpForm(p=>({...p,split:!p.split,splitWith:""}))} border={border}/>
            </div>
            {expForm.split&&(
              <div style={{marginTop:12}}>
                <input value={expForm.splitWith} onChange={e=>setExpForm(p=>({...p,splitWith:e.target.value}))} placeholder="Split with who? (name)"/>
                {expForm.splitWith&&expForm.amount&&(
                  <div style={{marginTop:8,background:"#6366f110",borderRadius:8,padding:"8px 12px",fontSize:13,color:"#6366f1",fontWeight:500}}>Each pays ₹{Math.round(+expForm.amount/2).toLocaleString()}</div>
                )}
              </div>
            )}
          </div>
          ...
          <button className="btn-p" onClick={saveExp}>{editExpId?"Save changes":"Save expense"}</button>
          {editExpId&&<button onClick={()=>{deleteExp(editExpId);setSheet(null);}} style={{color:"#ef4444",fontSize:14,fontWeight:600,padding:8}}>Delete expense</button>}
        </div>
      </Sheet>
      ...
      <Sheet open={sheet==="note"} onClose={()=>setSheet(null)} title={editNoteId?"Edit note":"New note"}>
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div><Lbl>Tags</Lbl><input value={noteForm.tags} onChange={e=>setNoteForm(p=>({...p,tags:e.target.value}))} placeholder="work, ideas, personal (comma separated)"/></div>
          <div>
            <Lbl>Color</Lbl>
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              {NOTE_COLORS.map(c=>(
                <button key={c} onClick={()=>setNoteForm(p=>({...p,color:c}))} style={{width:32,height:32,borderRadius:"50%",background:c,border:`3px solid ${noteForm.color===c?"#fff":"transparent"}`,outline:`2px solid ${noteForm.color===c?c:"transparent"}`,transition:"all .15s"}}/>
              ))}
            </div>
          </div>
          <button className="btn-p" onClick={saveNote}>{editNoteId?"Update note":"Save note"}</button>
          {editNoteId&&<button onClick={()=>deleteNote(editNoteId)} style={{color:"#ef4444",fontSize:14,fontWeight:600,padding:8}}>Delete note</button>}
        </div>
      </Sheet>
    </>
  );
